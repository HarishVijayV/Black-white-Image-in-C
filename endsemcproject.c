...
int main() {
    ...
    unsigned char *image;
    char str[100];//creating a character unsigned array with name str
    printf("Enter the name of the file (with .ppm) :");//getting the input as name of the image 
    gets(str);//storing the name of input image in str array
    // Read image from file
    FILE *in_file = fopen(str, "rb");//now *in file open the image in rb format (rb=read binary)
    fscanf(in_file, "P6\n%d %d\n%d\n", &width, &height, &max_color);//from in file(input image.ppm) scan the values
    image = (unsigned char *) malloc(width * height * 3);//(allocate)create same(dimensional) space in new array*(pointer) 'image' at that address
